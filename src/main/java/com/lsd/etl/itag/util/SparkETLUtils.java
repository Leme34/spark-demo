...
public class SparkETLUtils {
    ...
    /**
     * 配置操作Hive的SparkSession，一个应用中只能有一个SparkSession
     */
    public static SparkSession initSparkSession4Hive() {
        // Setting Master for running it
        SparkConf sparkConf = new SparkConf()
                .setAppName("UserHeatETL")
//                .setJars(new String[]{"/spark-demo1-1.0-SNAPSHOT.jar"})
...
//                .setJars(new String[]{"/spark-itags-1.0-SNAPSHOT.jar"})  //设置分发到集群的jar，非local模式必须配置否则ClassCastException
     ...
     * @return List<T>
     */
    public static <T> List<T> execAndCollectAsList(SparkSession sparkSession, String sql, Class<T> clazz) {

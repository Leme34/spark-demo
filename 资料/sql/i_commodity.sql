...
CREATE TABLE `t_commodity` (
  ...
  `commodity_price` decimal(10,2) DEFAULT NULL  COMMENT '商品金额',
  `commodity_cate_one` int(11) DEFAULT NULL COMMENT '商品一级分类',
  `commodity_cate_two` int(11) DEFAULT NULL COMMENT '商品二级分类',
...
CREATE TABLE `t_commodity_cate` (
  `id` int(11) NOT NULL,
  `cate_name` varchar(50) DEFAULT NULL COMMENT '类目ID',
  `cate_parent_id` int(11) DEFAULT NULL COMMENT '父类别ID（一级类目为0）',
  `create_user_id` int(11) DEFAULT NULL COMMENT '创建人（后台用户ID）',
  `status` int(4) DEFAULT NULL COMMENT '状态（0:禁用 1:启用）',
  `create_time` datetime DEFAULT NULL,
  `update_time` datetime DEFAULT NULL,
